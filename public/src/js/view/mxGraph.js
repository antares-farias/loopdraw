...
mxGraph.prototype.insertRelationEdge = function(parent, id, value, source, target, style, info)
{
	
	var edge = this.createEdge(parent, id, value, source, target, style);
	
	var fill = (info.number == "hasMany" || info.number == "hasAndBelongsToMany")?'1':"0";
	var dashed  = (info.tag == "hasManyThrough")?'1':"0";
	var end = "open";
	var color = "#000000";
	if (info.tag == "hasManyThrough" || info.tag == "hasAndBelongsToMany"){
		end  = 'diamondThin';
	}
	...
	assoc.edge = true;
	/*var number = (info.number == "hasMany")?'0..n':"1:1";
	var sourceLabel = new mxCell(number, new mxGeometry(-1, 0, 0, 0), 'resizable=0;align=left;verticalAlign=top;labelBackgroundColor=#ffffff;fontSize=10');
	sourceLabel.geometry.relative = true;
	sourceLabel.setConnectable(false);
	sourceLabel.vertex = true;
	assoc.insert(sourceLabel);
	
	var targetLabel = new mxCell(number, new mxGeometry(1, 0, 0, 0), 'resizable=0;align=right;verticalAlign=top;labelBackgroundColor=#ffffff;fontSize=10');
	targetLabel.geometry.relative = true;
	targetLabel.setConnectable(false);
	targetLabel.vertex = true;
	assoc.insert(targetLabel);*/
	//graph.insertCell(assoc);
	return this.addEdge(assoc, parent, source, target);
};
/**
